<template>
    <div>
        ...
        <div class="params" v-if="!isLoading">
            ...
            <a @click="sort('word', true)"><font-awesome-icon icon="sort-alpha-down" /> </a>
            <a @click="sort('word', false)"><font-awesome-icon icon="sort-alpha-up" /> </a>
        </div>
        <div v-if="isNewWord" class="add-word">
            Le mot {{search}} n'a pas encore été proposé.
            <br>
            <a class="add-button" @click="onAdd">Ajouter {{search}}</a>
        </div>
        <div class="word-list" v-if="!isLoading">
            <div class="word-item" v-for="word in filteredWords" :key="word.word">
                ...
                <!-- <div class="stars" v-if="canVote(word)">
                    <span v-for="value in [5,4,3,2,1]" :key="value"
                    @click="vote(word, value)">☆</span>
                </div>
                ...
                <a class="vote-button" v-if="canVote(word)" @click="openModal(word)">
                    <font-awesome-icon icon="vote-yea" />
                </a>
                <div class="a-vote" v-else>
                    <font-awesome-icon icon="vote-yea" />
